...
function switchAdminTab(tabId) {
  document.querySelectorAll('.tab-section').forEach(sec => {
    sec.style.setProperty(
      'display',
      sec.id === `section_${tabId}` ? 'block' : 'none',
      'important'
    );
  });

  if (tabId === 'my_salary') {
    const salarySection = document.getElementById('section_my_salary');
    const staffSalaryView = document.getElementById('staffSalaryView');

    if (salarySection) {
      salarySection.style.setProperty('display', 'block', 'important');
    }

    if (staffSalaryView) {
      staffSalaryView.style.setProperty('display', 'block', 'important');
    }
  }
  ...
  const salarySection =
    document.getElementById('section_my_salary');
...
async function openAddPaymentModal() {
    ...
    try {
        ...
        console.error('[PAYMENT] Lỗi tải dữ liệu tạo hóa đơn:', error);
        showToast('Không thể tải dữ liệu tạo hóa đơn!', 'error');
    }
}
...
async function loadMySalary() {
  ...
  const trainersResult =
    await GymAPI.getTrainers();

  const trainers =
    Array.isArray(trainersResult)
      ? trainersResult
  ...
  let trainerId =
    Number(currentUser.TrainerID || 0);

  if (!trainerId) {
    const currentTrainer =
      trainers.find(t =>
        Number(t.UserID) === Number(currentUser.UserID) ||
        String(t.FullName || '').trim().toLowerCase() ===
          String(currentUser.Fullname || '').trim().toLowerCase()
      );

    if (currentTrainer) {
      trainerId =
        Number(currentTrainer.TrainerID);
    }
  }
  ...
  const completedThisMonth =
    completedBookings.filter(b =>
      String(b.BookingDate || '').startsWith(currentMonth)
    );
  ...
  const salaries =
    Array.isArray(salariesResult)
      ? salariesResult
      : (Array.isArray(salariesResult?.data)
          ? salariesResult.data
          : []);

  const currentSalary =
    salaries.find(s =>
      Number(s.UserID) === Number(currentUser.UserID) &&
      String(s.SalaryMonth || '') === currentMonth
    );
  ...
  const incomeEl =
    document.getElementById('trainerIncomeTotal');

  const sessionsEl =
    document.getElementById('trainerIncomeSessions');

  const averageEl =
    document.getElementById('trainerIncomeAverage');

  if (incomeEl) {
    incomeEl.textContent =
      formatVND(salaryAmount);
  }
  ...
  if (averageEl) {
    averageEl.textContent =
      sessionCount > 0
        ? `${formatVND(Math.round(averagePerSession))}/buổi`
        : '0đ/buổi';
  }

  // ============================================================
  // BẢNG BUỔI DẠY GẦN ĐÂY
  // ============================================================

  const incomeTable =
    document.getElementById('trainerIncomeTableBody');
      ...
      completedBookings
        .sort((a, b) =>
          String(b.BookingDate || '')
            .localeCompare(String(a.BookingDate || ''))
        )
        .slice(0, 10);
    ...
    incomeTable.innerHTML =
      recentBookings.length > 0
        ? recentBookings.map(b => `
            <tr>
              <td style="color:#9CA3AF; padding:14px 18px;">
              ...
              <td style="font-weight:700; color:#FF334B; text-align:right; padding:14px 18px;">
                ${
                  averagePerSession > 0
                    ? formatVND(Math.round(averagePerSession))
                    : '—'
                }
              </td>
            </tr>
  ...
  const chart =
    document.getElementById('trainerIncomeChart');
  ...
  if (chart) {
    ...
    chart.innerHTML =
      monthValues.map((value, index) => {
        ...
        const height =
          value > 0
            ? Math.max(20, (value / maxValue) * 180)
            : 4;

        const month =
          Number(months[index].slice(5));
  ...
  return;
    ...
    const totalEl =
      document.getElementById('mySalaryAmount');
...
async function loadTrainerAttendance() {
  try {
    ...
    const monthRecords = records.filter(r =>
      String(r.ShiftDate || r.AttendanceDate || '')
    ...
    const workDays = new Set(
      monthRecords
        ...
        .map(r => String(r.ShiftDate || r.AttendanceDate).substring(0, 10))
    ).size;
    ...
    const sessionCount =
      monthRecords.filter(r => r.CheckInTime).length;

    const workDaysEl =
      document.getElementById('trainerAttWorkDays');

    const lateDaysEl =
      document.getElementById('trainerAttLateDays');

    const sessionsEl =
      document.getElementById('trainerAttTotalSessions');
    ...
    if (lateDaysEl) {
      lateDaysEl.textContent = `${lateDays} Lần`;
    }

    if (sessionsEl) {
      sessionsEl.textContent = `${sessionCount} Ca`;
    }

    const tableBody =
      document.getElementById('myAttendanceTableBody');
      ...
      records.map(r => {
        ...
        return `
          <tr style="border-bottom:1px solid rgba(255,255,255,0.06);">
            ...
            <td style="font-weight:700; color:#FFFFFF; text-align:center; padding:14px 18px;">
              ${currentUser.UserID}
            </td>
    ...
    if (tableBody) {
      tableBody.innerHTML = `
        <tr>
          <td colspan="5"
              style="text-align:center; padding:30px; color:#EF4444;">
            Không thể tải dữ liệu chấm công.
          </td>
        </tr>
      `;
...
async function handleStaffRealtimeCheckIn() {
  try {
    ...
    // Quan trọng:
    // Tìm BẤT KỲ ca nào của hôm nay,
    // kể cả ca đã checkout.
    const todayRecord =
      records.find(r =>
        String(
          r.ShiftDate ||
    ...
    if (todayRecord) {
      ...
      if (!todayRecord.CheckOutTime) {
        ...
        showToast(
          `Bạn đang trong ca! Đã check-in lúc ${todayRecord.CheckInTime}.`,
          'error'
        );
        ...
        showToast(
          `Hôm nay bạn đã chấm công lúc ${todayRecord.CheckInTime}. Không thể check-in lần 2.`,
          'error'
        );
    ...
    showToast(
      `Check-in vào ca thành công lúc ${timeStr}!`,
      'success'
    );
    ...
    try {
        const currentUser =
            (typeof GymAPI !== 'undefined' && GymAPI.getCurrentUser)
                ? GymAPI.getCurrentUser()
                : null;
        const trainerActionBtn =
            document.getElementById(
                'trainerAttendanceActionBtn'
            );
        ...
        if (currentShift) {
            ...
            // =========================
            if (trainerActionBtn) {
                trainerActionBtn.disabled = false;
                trainerActionBtn.innerHTML =
                    '<i class="fa fa-sign-out-alt"></i> Check-out Tan Ca';
                trainerActionBtn.style.background =
                    'linear-gradient(135deg, #FF334B 0%, #DC2626 100%)';
            }
            if (statusEl) {
            ...
            if (trainerActionBtn) {
                trainerActionBtn.disabled = false;
                trainerActionBtn.innerHTML =
                    '<i class="fa fa-fingerprint"></i> Check-in Hôm Nay';
                trainerActionBtn.style.background =
         ...
         if (
                ...
                trainerActionBtn.disabled = true;
                trainerActionBtn.innerHTML =
                    '<i class="fa fa-check-double"></i> Đã chấm công hôm nay';
                trainerActionBtn.style.background =
...
async function handleTrainerAttendanceAction() {
  try {
    const currentUser =
      (typeof GymAPI !== 'undefined' && GymAPI.getCurrentUser)
        ? GymAPI.getCurrentUser()
    ...
    if (!todayRecord) {
      await handleStaffRealtimeCheckIn();
      return;
    }

    // Đang trong ca -> Checkout
    if (!todayRecord.CheckOutTime) {
      const attendanceId =
        todayRecord.EmployeeAttendanceID ??
        todayRecord.AttendanceStaffID ??
        todayRecord.AttendanceID ??
        todayRecord.ID;

      if (!attendanceId) {
        showToast(
          'Không tìm thấy mã ca để checkout!',
          'error'
        );
        return;
      }
      ...
      const checkoutResult =
        await GymAPI.checkOutStaff(attendanceId);
    ...
    // Đã checkout rồi
    showToast(
      `Hôm nay đã hoàn thành ca lúc ${todayRecord.CheckInTime}.`,
      'info'
    );
    ...
    console.error(
      '[TRAINER ATTENDANCE ACTION ERROR]',
      error
    );

    showToast(
      'Không thể thực hiện thao tác chấm công!',
      'error'
    );
  }
}
...
async function handleTrainerAttendanceAction() {
  try {
    ...
    // Chưa có record hôm nay → CHECK-IN
    if (!todayRecord) {
    ...
    // Có record nhưng chưa checkout → CHECK-OUT
    if (!todayRecord.CheckOutTime) {
    ...
    // Đã checkout → không cho check-in lần 2
    showToast(
      'Hôm nay bạn đã hoàn thành ca, không thể check-in lần 2.',
      'info'
    );
...
window.handleTrainerAttendanceAction =
  handleTrainerAttendanceAction;

window.loadTrainerAttendance =
  loadTrainerAttendance;