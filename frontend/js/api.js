...
// ============================================================================
// 4. NHÓM CHỨC NĂNG ĐIỂM DANH HỘI VIÊN (ATTENDANCE)
// ============================================================================
...
async getAttendance() {
    try {
        ...
        if (!result.success) {
            throw new Error(
                result.message || 'Không thể lấy dữ liệu điểm danh'
            );
        }
...
async checkIn(memberId, type = 'Gym & Fitness') {
    try {
        ...
        console.error('Check-in Attendance API Error:', error);
...
async checkOut(attendanceId) {
    try {
        const response = await fetch(
            `${API_CONFIG.BASE_URL}/attendance.php?id=${Number(attendanceId)}`,
            {
                method: 'PUT',
                credentials: 'include',
                headers: {
        ...
        console.error('Check-out Attendance API Error:', error);
...
async deleteMemberAttendance(attendanceId) {
    try {
        const response = await fetch(
            `${API_CONFIG.BASE_URL}/attendance.php?id=${Number(attendanceId)}`,
            {
...
async getBookings() {
  try {
    const response = await fetch(
      `${API_CONFIG.BASE_URL}/booking.php`,
      {
        method: 'GET',
        credentials: 'include'
      }
    ...
    if (!response.ok || !result.success) {
      console.error('Get Bookings Error:', result);
...
async updateBookingStatus(id, status, attendanceStatus) {
  try {
    ...
    const response = await fetch(
      `${API_CONFIG.BASE_URL}/booking.php?id=${Number(id)}`,
      {
        method: 'PUT',
        credentials: 'include',
        headers: {
    ...
    console.error(
      'Update Booking Status API Error:',
      error
    );
  ...
  async getTodaySales() {
    try {
        ...
        if (!response.ok || !result.success) {
            console.error('Get Today Sales Error:', result);
            return [];
        }
        ...
        console.error('Get Today Sales API Error:', error);
        return [];
    }
},
...
async deletePayment(id) {
  ...
  async getInventory() {
    try {
        ...
        if (!result.success) {
            throw new Error(
                result.message || 'Không thể tải thông tin sản phẩm!'
            );
        }
...
async getFeedbacks(trainerId = null) {
      ...
      let url =
          `${API_CONFIG.BASE_URL}/feedback.php`;

      if (trainerId) {
          url += `?trainerID=${Number(trainerId)}`;
      }
